...
const bottomItems = [
  ...
  { id: 'settings', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 012-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06A1.65 1.65 0 0 0 9 4.6a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 012 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>', label: 'Settings', tooltip: 'Settings — Configure application preferences, account, and workspace options' },
];
...
export function Sidebar({ onNavigate }: SidebarProps) {
  ...
  useEffect(() => {
    const handleRouteChange = (e: CustomEvent) => {
      ...
      if (page.startsWith('template/')) page = 'apps';
      setActiveRoute(page);
    };
    ...
    if (path) {
      const activePage = path.startsWith('template/') ? 'apps' : path;
      setActiveRoute(activePage);
    }
    return () => window.removeEventListener('route-changed', handleRouteChange as EventListener);
  }, []);
  ...
  const renderButton = (item: { id: string; icon: string; label: string; tooltip: string }, isActive: boolean) => {
    const isImage = item.id === 'image';
    const showActiveStyle = isActive || isImage;

    return (
      <div
        key={item.id}
        className="flex flex-col items-center gap-0.5 mb-1 cursor-pointer group w-full px-2"
        data-route={item.id}
        ...
        <button
          className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
            showActiveStyle
              ...
              : 'bg-transparent text-secondary group-hover:bg-white/5 group-hover:text-white'
          }`}
  ...
  return (
    <aside
      className="hidden md:flex flex-col items-center py-4 z-40 border-r border-white/5 bg-panel-bg overflow-y-auto custom-scrollbar"
      style={{ width: '68px' }}
    >
      <div className="flex flex-col flex-1 w-full items-center pt-2 gap-1">
        {navItems.map(item => renderButton(item, activeRoute === item.id))}
      </div>
      <div className="flex flex-col w-full items-center mt-auto">
        {bottomItems.map(item => renderButton(item, activeRoute === item.id))}
      </div>
    </aside>
  );
}