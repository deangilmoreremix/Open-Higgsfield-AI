#!/usr/bin/env python3
"""Generate HTML files for dedicated apps and modules"""

import os

BASE = "/workspace/0c85e0dc-1244-40ab-8f84-e11668f857da/sessions/agent_b64f4b8e-d076-49a3-a9ae-6ff7715bbd4e/demo-videos"

# Dedicated apps: (folder, title, feature1, feature2, feature3)
APPS = [
    ("apps-ai-storyboarder", "AI Storyboarder", "Scene Creation", "AI Generation", "Timeline"),
    ("apps-ai-vfx", "AI VFX", "Effects Library", "Preview", "Apply"),
    ("apps-director", "Director", "Scene Director", "Camera Angles", "Timeline"),
    ("apps-remix-go", "Remix-Go", "Music Sync", "Beat Detection", "Export"),
    ("apps-sendspark", "Sendspark", "Personalization", "Batch Processing", "Analytics"),
    ("apps-vimax", "Vimax", "Video Editor", "Multi-Track", "Effects"),
]

# Modules: (folder, title, feature1, feature2, feature3)
MODULES = [
    ("mod-rendiv", "Rendiv", "Animation System", "Keyframes", "Rendering"),
    ("mod-ltx-desktop", "LTX-Desktop", "Desktop App", "Local Processing", "Export"),
    ("mod-cinegen", "CineGen", "Cinema Generation", "AI Scripts", "Rendering"),
    ("mod-chatvideo", "ChatVideo", "Video Chat", "AI Assistant", "Generation"),
]
...
# Generate dedicated apps
for folder, title, f1, f2, f3 in APPS:
    app = folder.replace("apps-", "")
...
# Generate modules
for folder, title, f1, f2, f3 in MODULES:
    app = folder.replace("mod-", "")
    html = HTML.format(app=app, title=title, f1=f1, f2=f2, f3=f3)
    filepath = os.path.join(BASE, folder, "index.html")
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    with open(filepath, 'w') as f:
...
print("\nAll dedicated apps and modules HTML files generated!")
