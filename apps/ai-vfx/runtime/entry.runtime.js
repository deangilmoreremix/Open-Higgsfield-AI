import { AIVFXRuntimeAdapter } from './adapter.js';

export function mountNative(container, options = {}) {
  const adapter = new AIVFXRuntimeAdapter(options);

  const appUI = document.createElement('div');
  appUI.className = 'w-full h-full flex flex-col bg-app-bg';
  appUI.innerHTML = `
    <div class="p-4 border-b border-white/10">
      <h1 class="text-lg font-bold text-white">AI VFX</h1>
      <p class="text-sm text-secondary">Video effects and visual generation</p>
    </div>
    <div id="runtime-status" class="p-4">
      <p class="text-xs text-secondary">Text-to-video and image-to-video generation</p>
    </div>
  `;
